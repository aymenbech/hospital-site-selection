...
export default function DecisionMakers() {
  ...
  const processedDatasetId = searchParams.get('processedDatasetId')
  ...
  function handleSelectDecisionMaker(dmId: string) {
    ...
    const suffix = processedDatasetId ? `&processedDatasetId=${processedDatasetId}` : ''
    navigate(`/projects/${projectId}/ahp-matrix?datasetId=${datasetId}&expertId=${dmId}${suffix}`)
  }

  function handleGoToWAMAggregation() {
    if (!datasetId) { setError('Please select a dataset first.'); return }
    const suffix = processedDatasetId ? `&processedDatasetId=${processedDatasetId}` : ''
    navigate(`/projects/${projectId}/wam-aggregation?datasetId=${datasetId}${suffix}`)
  }
  ...
  return <main className="max-w-6xl mx-auto p-8">
    ...
    <section className="mt-6 bg-white border rounded-lg p-6 shadow-sm">
      ...
      <p className="mt-2 text-gray-600">Each expert completes an independent AHP assessment using the same 7 database-backed criteria.</p>
      {datasetId && <p className="mt-2 text-sm text-blue-600">Raw dataset: <strong>{datasetId.slice(0, 8)}...</strong>{processedDatasetId && <> · Processed dataset: <strong>{processedDatasetId.slice(0, 8)}...</strong></>}</p>}
    </section>
