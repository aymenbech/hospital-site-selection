...
from app.models.dataset import Criterion, ProcessedDataset
from app.models.project import Project
from app.schemas.wam import (
    WAMAnalysisRequest,
    WAMAnalysisResponse,
    WAMExpertScore,
    WAMRankingResult,
...
def execute_wam(payload: WAMAnalysisRequest, db: Session = Depends(get_db)):
    ...
    if processed.raw_dataset_id not in [dataset.id for dataset in project.datasets]:
        raise HTTPException(status_code=400, detail="Processed dataset does not belong to this project")

    criteria = (
        db.query(Criterion)
        .filter(Criterion.project_id == payload.project_id, Criterion.is_active == True)
        .all()
    )
    ...
    if len(criteria) != 7:
        raise HTTPException(
            status_code=400,
            detail=f"The final methodology requires exactly 7 active criteria; found {len(criteria)}.",
        )
    ...
    # Build the normalized decision matrix once. Normalization is independent
    # of the expert; only the AHP weights vary between experts.
    zone_rows = []
    ...
    for zone in processed.data:
        ...
        for criterion in criteria:
            value = as_float(
                get_zone_value(zone, criterion.source_column),
                f"{criterion.code} in zone {zone_id}",
            )
    ...
    stats = {
        cid: (min(values), max(values))
        for cid, values in raw_values.items()
    }
    ...
    for expert in payload.experts:
        if expert.consistency_ratio > 0.10:
            raise HTTPException(
                status_code=400,
                detail=f"Expert '{expert.expert_name}' has CR={expert.consistency_ratio:.4f}; CR must be <= 0.10.",
            )

        if len(expert.criteria_weights) != 7:
            raise HTTPException(
                status_code=400,
                detail=f"Expert '{expert.expert_name}' must provide weights for exactly 7 criteria.",
            )
        ...
        if set(weights) != criterion_ids:
            raise HTTPException(
                status_code=400,
                detail=f"Expert '{expert.expert_name}' criteria weights do not match the 7 active project criteria.",
            )
        ...
        if total_weight <= 0 or abs(total_weight - 1.0) > 0.001:
            raise HTTPException(
                status_code=400,
                detail=f"Expert '{expert.expert_name}' AHP weights must sum to 1 (found {total_weight:.6f}).",
            )

        zone_scores = {}
        for zone_id, _, normalized in normalized_rows:
            zone_scores[zone_id] = sum(
                weights[criterion.id] * normalized[criterion.id]
                for criterion in criteria
            )
    ...
    # Equal expert weighting: arithmetic mean of the per-expert WAM scores.
    final_scores = {}
    zone_names = {zone_id: zone_name for zone_id, zone_name, _ in normalized_rows}
    for zone_id, _, _ in normalized_rows:
        final_scores[zone_id] = sum(
            result.zone_scores[zone_id] for result in expert_results
        ) / len(expert_results)
