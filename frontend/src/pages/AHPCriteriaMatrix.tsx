...
export default function AHPCriteriaMatrix() {
  ...
  const datasetId = searchParams.get('datasetId'); const expertId = searchParams.get('expertId')
  const [criteria, setCriteria] = useState<Criterion[]>([]); const [expertName, setExpertName] = useState('')
  const [matrix, setMatrix] = useState<ComparisonMatrix>({}); const [weights, setWeights] = useState<Weight[]>([])
  const [lambdaMax, setLambdaMax] = useState<number | null>(null); const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null); const [saving, setSaving] = useState(false); const [cr, setCr] = useState<number | null>(null)

  useEffect(() => {
    if (!datasetId) { setError('Dataset ID is missing from the URL.'); setLoading(false); return }
    try {
      const raw = localStorage.getItem(`criteria_${datasetId}`); if (!raw) throw new Error('No criteria found. Please select criteria first.')
      const parsed = JSON.parse(raw) as Criterion[]; if (parsed.length !== 7) throw new Error(`The final methodology requires exactly 7 criteria. Currently selected: ${parsed.length}.`)
      setCriteria(parsed)
      const saved = expertId ? localStorage.getItem(`ahp_matrix_${datasetId}_${expertId}`) : null
      if (saved) { const p = JSON.parse(saved); setMatrix(p.matrix || {}); if (p.expertName) setExpertName(p.expertName) }
      else { const initial: ComparisonMatrix = {}; parsed.forEach(r => { initial[r.id] = {}; parsed.forEach(c => { initial[r.id][c.id] = 1 }) }); setMatrix(initial) }
  ...
  useEffect(() => {
    ...
    const ids = criteria.map(c => c.id); const n = 7
    ...
    const ci = (lambda - n) / 6; const ratio = ci / RANDOM_INDEX[7]
    setLambdaMax(lambda); setCr(Math.max(0, ratio)); setWeights(ids.map((id, i) => ({ criterion_id: id, criterion_name: `${criteria[i].name} (${criteria[i].code})`, weight: priority[i] })))
  }, [criteria, matrix])
  ...
  async function handleSave() {
    ...
    if (criteria.length !== 7 || cr === null || cr > 0.10) { setError(`Exactly 7 criteria and CR ≤ 0.10 are required. Current CR: ${cr?.toFixed(4) ?? 'N/A'}`); return }
    const comparisons = []; for (let i = 0; i < criteria.length; i++) for (let j = i + 1; j < criteria.length; j++) comparisons.push({ criterion_i_id: criteria[i].id, criterion_j_id: criteria[j].id, scale_value: matrix[criteria[i].id]?.[criteria[j].id] ?? 1 })
    if (comparisons.length !== 21) { setError('AHP requires exactly 21 pairwise comparisons.'); return }
    setSaving(true); setError(null)
    try {
      const { data: { session } } = await supabase.auth.getSession(); if (!session?.access_token) throw new Error('You must sign in before saving AHP.')
      const response = await fetch(`${API_BASE_URL}/ahp/runs`, { method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ project_id: projectId, analysis_run_id: null, name: `${expertName} — AHP`, description: `AHP criteria weights for decision maker ${expertId}`, comparisons }) })
      const body = await response.json(); if (!response.ok) throw new Error(body.detail || `AHP save failed (${response.status})`)
      localStorage.setItem(`ahp_matrix_${datasetId}_${expertId}`, JSON.stringify({ comparisonRunId: body.comparison_run_id, decisionMakerId: expertId, criteria, matrix, weights, lambdaMax, consistencyRatio: cr, expertName, savedAt: new Date().toISOString() }))
  ...
  return <main className="mx-auto max-w-7xl p-8">
    ...
    <section className="mt-6 rounded-lg border bg-white p-6 shadow-sm"><h1 className="text-2xl font-bold">AHP Pairwise Comparison Matrix — 7 Criteria</h1><p className="mt-2 text-gray-600">Decision maker: <input value={expertName} onChange={e => setExpertName(e.target.value)} className="border rounded px-2 py-1 font-semibold" /></p><p className="mt-2 text-sm text-gray-500">Seven criteria produce exactly 21 independent pairwise comparisons. Reciprocal values are generated automatically.</p></section>
